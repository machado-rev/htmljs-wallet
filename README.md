...
This is a client-side wallet library that can generate private keys from a mnemonic, or import private keys from other  wallets.

It can sign transactions locally, and submit the raw transaction data to a remote htmlcoin node. The blockchain data is provided by the Insight API (which powers https://explorer.htmlcoin.com/), rather than the raw htmlcoind RPC calls.

This library makes it possible to run DApp without the users having to run a full htmlcoind node.

> This library is extracted from the official [HTMLCOIN web wallet](https://github.com/machado-rev/htmlcoin-web-wallet).
...
- Uses satoshi (1e8) as internal units
  - Can represent up to ~90 million HTML accurately.
...
The currency unit used is `satoshi`. To convert html to satoshi you should multiply the amount you want with `1e8`.

```js
import { networks } from "htmljs-wallet";
...
The ABI encoding for the `burnbabyburn()` invokation is `e179b912`. We'll burn 0.05 html, expressed in unit of satoshi.
...
async function main() {
  ...
  const tx = await wallet.contractSend(contractAddress, encodedData, {
    amount: 0.05 * 1e8 // 0.05 html in satoshi
  });
...
```
htmlcoin-cli dumpprivkey qg3HYD8c4bAVLeEzA9t3Ken3Y3Mni1HZSS
...
```ts
const toAddress = "hZaTYNEimGLuqnBDpP3KvBKsFs3DbCuwnr";
const amount = 0.15 * 1e8; // 0.15 HTML
...
```ts
const tx = await wallet.send(toAddress, amount, {
  // rate is 400 satoshi per byte, or  ~0.004 html/KB, as is typical.
  feeRate: 400
});
```
...
You can decode the raw transaction using `htmlcoin-cli`:

```
htmlcoin-cli decoderawtransaction 0100000001a09a921ecb38...
...
```
htmlcoin-cli decoderawtransaction 0100000001e7b6f2767e8a6...
```
